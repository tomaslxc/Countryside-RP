...
function getMatrixFromPoints(x,y,z,x3,y3,z3,x2,y2,z2)
	...
	y2 = z3*x1-x3*z1
	z2 = x3*y1-y3*x1
	local c1=x1*x1+y1*y1+z1*z1
	local c2=x2*x2+y2*y2+z2*z2
	local c3=x3*x3+y3*y3+z3*z3
	if c1==0 then c1=1 end
	if c2==0 then c2=1 end
	if c3==0 then c3=1 end
	local len1 = 1/math.sqrt(c1)
	local len2 = 1/math.sqrt(c2)
	local len3 = 1/math.sqrt(c3)
...
if not script_serverside_fixed then
	function getBoneMatrix(ped,bone)
		...
		x,y,z = getPedBonePosition(ped,bone_0_fixed[bone])
		if bone == 1 then
		...
		elseif bone == 3 then
		  if not (getElementModel(ped) == 0) then
			...
			tx,ty,tz = getPedBonePosition(ped,bone_t_fixed[bone])
		end
		fx,fy,fz = getPedBonePosition(ped,bone_f_fixed[bone])
